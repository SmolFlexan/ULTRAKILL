...
public struct NativePortalScene : IDisposable
{
	...
	{
		if (portals.IsCreated && handleToIndex.TryGetValue(handle.PackedKey, out var item))
		{
			return ref portals.GetUnsafeReadOnlyPtr()[item];
		}
	...
	public static NativePortalScene Create(List<Portal> portalComponents, List<PortalIdentifier> identifiers, Dictionary<PortalHandle, PortalIdentifier> portalIdentifiersLookup)
	{
		...
		{
			portals = new NativeList<NativePortal>(count * 2, Allocator.TempJob),
			renderPortals = new NativeList<NativePortal>(count * 2, Allocator.TempJob),
			handleToIndex = new UnsafeHashMap<long, int>(count * 2, Allocator.TempJob)
		};
		...
		for (int i = 0; i < count2; i++)
		{
			...
			if (portal.mirror)
			{
				result.renderPortals.Add(in enter);
				continue;
			}
			...
			enter.audioData = new NativePortalAudioData
			{
				...
				audioVelocityUpdateMode = AudioVelocityUpdateMode.Auto,
			...
			result.portals.Add(in enter);
			result.renderPortals.Add(in enter);
			result.handleToIndex[exitHandle.PackedKey] = length + 1;
			result.portals.Add(in exit);
			result.renderPortals.Add(in exit);
		}
		result.valid = true;
		return result;
	}
