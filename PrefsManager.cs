	...
	private readonly Dictionary<string, Func<object, object>> propertyValidators = new Dictionary<string, Func<object, object>>
	{
		...
		},
		{
			...
			delegate(object value)
			{
				...
				if (!(value is int num))
				{
					Log.Warning("CyberGrindStartingWave value is not an int");
					return 0;
				}
				...
				{
					Log.Warning("No wave data found for difficulty level");
					return 0;
				}
				if (!WaveUtils.IsWaveSelectable(num, highestWaveForDifficulty.Value))
				{
					Log.Warning($"Wave {num} is not unlocked yet. Highest wave: {highestWaveForDifficulty.Value}");
					return 0;
				}
				return (object)null;
			}
