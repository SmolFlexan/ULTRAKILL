...
public class NewMovement : MonoSingleton<NewMovement>, ITarget, IPortalTraveller
{
	...
	private void SlideCancelled(InputAction.CallbackContext ctx)
	{
		if (sliding)
		{
			slideTimestamp = ctx.time;
		}
	}

	private void JumpPerformed(InputAction.CallbackContext ctx)
	{
		jumpTimestamp = ctx.time;
	}
	...
	public AudioSource DuplicateDetachWhoosh()
	{
		...
		audWoosh.enabled = false;
		GameObject obj = UnityEngine.Object.Instantiate(audWoosh.gameObject, audWoosh.transform.parent, worldPositionStays: true);
	...
	private void Update()
	{
		ParticleSystem.EmissionModule emission = windStateParticle.emission;
		windStateParticle.transform.position = cc.GetDefaultPos() + rb.velocity.normalized;
		windStateParticle.transform.LookAt(cc.GetDefaultPos());
		emission.rateOverDistanceMultiplier = Mathf.Min(windState, 0.5f) * 10f;
		if (gc.onGround)
		{
			CheckForGasoline();
		...
		{
			if (oilSlideEffect.gameObject.activeSelf)
			{
	...
	public bool? OnTravel(PortalTravelDetails details)
	{
		...
		if (gc.onGround)
		{
			...
			if (Physics.Raycast(vector9, -vector10, out var hitInfo2, num9 + 1f, LayerMaskDefaults.Get(LMD.Environment), QueryTriggerInteraction.Ignore))
			{
				...
				Vector3 forward = Vector3.ProjectOnPlane(nativePortalTransform.forward, up);
				Vector3 forward2 = Vector3.ProjectOnPlane(nativePortalTransform2.back, hitInfo2.normal);
