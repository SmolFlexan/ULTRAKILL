...
{
	internal delegate int CountPermutations_00002B9C_0024PostfixBurstDelegate(int count, int depth);

	internal static class CountPermutations_00002B9C_0024BurstDirectCall
	{
		...
		private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(CountPermutations_00002B9C_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		static CountPermutations_00002B9C_0024BurstDirectCall()
		{
			Constructor();
		}
	...
	{
		return CountPermutations_00002B9C_0024BurstDirectCall.Invoke(count, depth);
	}
	...
	public void UpdateData()
	{
		...
		targetStream = new NativeStream(visions.Count, Allocator.TempJob);
		if (!targetsList.IsCreated)
		{
			targetsList = new NativeList<TargetIndexAndDistance>(Allocator.Persistent);
		}
		targetsList.Clear();
		...
		targetStream.Dispose(outputVisionHandle);
