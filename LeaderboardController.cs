...
public class LeaderboardController : MonoSingleton<LeaderboardController>
{
	...
	public async Task<LeaderboardEntry[]> GetFishScores(LeaderboardType type)
	{
		...
		return (await FetchLeaderboardEntries("Fish Size", type, 20)).Where((LeaderboardEntry fs) => fs.Score == 1 || Enumerable.Contains(SteamController.BuiltInVerifiedSteamIds, fs.User.Id.Value)).Take(20).ToArray();
	}
	...
	public async void SubmitFishSize(int fishSize)
	{
		...
		{
			Leaderboard value = leaderboard.Value;
			if (!Enumerable.Contains<ulong>(SteamController.BuiltInVerifiedSteamIds, SteamClient.SteamId))
			{
