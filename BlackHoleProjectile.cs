...
public class BlackHoleProjectile : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (fadingIn)
		{
			...
			aud.SetPitch(1f - Vector3.Distance(base.transform.localScale, origScale) / origScale.magnitude);
		...
		if (collapsing)
		{
			if (aud.GetPitch() > 0f)
			{
				aud.SetPitch(aud.GetPitch() - Time.deltaTime);
			}
			else if (aud.GetPitch() != 0f)
			{
				aud.SetPitch(0f);
			}
		...
		else
		{
			...
			if (!enemy)
			{
				aud.SetPitch(aud.GetPitch() + Time.deltaTime / 2f);
			}
			if (power < 3f)
			{
			...
			foreach (Rigidbody caught2 in caughtList)
			{
				...
				list.Add(caught2);
				caught2.SetGravityMode(useGravity: false);
	...
	private void ShootRandomLightning()
	{
		...
		for (int i = 0; i < num; i++)
		{
			if (Physics.Raycast(base.transform.position, Random.insideUnitSphere.normalized, out rhit, 8f * base.transform.localScale.x, LayerMaskDefaults.Get(LMD.Environment)))
			{
