	...
	private void FixedUpdate()
	{
		...
		if (!inAction && gce.onGround && !jumping && eid.target != null)
		{
			if (cooldown > 0f)
			{
				...
				if (!stationary)
				{
					if (nma.enabled && nma.isOnNavMesh && Physics.Raycast(CurrentTargetPosition, Vector3.down, out var hitInfo3, float.PositiveInfinity, LayerMaskDefaults.Get(LMD.Environment)))
					{
						if (NavMesh.SamplePosition(CurrentTargetPosition, out var hit, 1f, nma.areaMask))
						{
