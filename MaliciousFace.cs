...
{
	private static readonly plog.Logger Log = new plog.Logger("MaliciousFace");
	...
	private void MovementUpdate()
	{
		...
		else if (eid.target != null)
		{
			...
			if (nma.CalculatePath(position, tempPath) && tempPath.status == NavMeshPathStatus.PathComplete)
			{
	...
	private void HandleSpiderDamage(ref DamageData data)
	{
		...
		if (eid.hitter != "fire" && !eid.sandified && !eid.blessed)
		{
			...
			if (eid.hitter != "shotgun" && eid.hitter != "drill" && base.gameObject.activeInHierarchy && dripBlood != null)
			{
				...
				if ((bool)currentDrip)
				{
					...
					if (MonoSingleton<BloodsplatterManager>.Instance.goreOn)
					{
						ParticleSystem component4 = currentDrip.GetComponent<ParticleSystem>();
						if ((bool)component4)
						{
							component4.Play();
						}
		...
		if ((bool)hurtSound && spider.health > 0f)
		{
			hurtSound.PlayClipAtPoint(MonoSingleton<AudioMixerController>.Instance.goreGroup, base.transform.position, 12, 1f, 0.75f, UnityEngine.Random.Range(0.85f, 1.35f));
		}
	}
