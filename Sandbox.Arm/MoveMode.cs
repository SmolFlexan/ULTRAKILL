...
public class MoveMode : ISandboxArmMode, ISandboxArmDebugGUI
{
	...
	private static readonly plog.Logger Log = new plog.Logger("MoveMode");
	...
	public bool OnGUI()
	{
		...
		GUILayout.Label("Computed Target Point: " + vector.ToString());
		GUILayout.Label("Object's (Pivot) Position " + manipulatedObject.target.position.ToString());
		vector = objectVelocity;
		GUILayout.Label("Velocity: " + vector.ToString());
		return true;
	}
	...
	public void OnSecondaryDown()
	{
		if (manipulatedObject != null && (!(manipulatedObject.spawnable != null) || !manipulatedObject.spawnable.disallowFreezing))
		{
			...
			hostArm.freezeSound.SetPitch(Random.Range(1f, 1.05f));
	...
	private void IntegrityCheck()
	{
		...
		{
			Log.Warning("Integrity check failed, releasing manipulated object");
