...
public class PortalAwarePlayerCollider : MonoBehaviour
{
	...
	private readonly ConcurrentDictionary<int, UnityEngine.Plane> colliderPortalPlanes = new ConcurrentDictionary<int, UnityEngine.Plane>();
	...
	private readonly List<(PortalHandle handle, UnityEngine.Plane plane)> activePortals = new List<(PortalHandle, UnityEngine.Plane)>();
	...
	private void FixedUpdate()
	{
		...
		for (int j = 0; j < count; j++)
		{
			...
			ref Unity.Mathematics.Geometry.Plane plane = ref nativePortal.plane;
			Unity.Mathematics.Geometry.Plane plane2 = plane;
			float num = plane2.SignedDistanceToPoint(position);
			if (num > 0f)
			{
				Vector3 velocity = rb.velocity;
				plane2 = plane;
				float num2 = 0f - Vector3.Dot(velocity, plane2.Normal);
			...
			List<(PortalHandle handle, UnityEngine.Plane plane)> list = activePortals;
			PortalHandle item = handle;
			plane2 = plane;
			Vector3 inNormal = plane2.Normal;
			plane2 = plane;
			list.Add((item, new UnityEngine.Plane(inNormal, plane2.Distance)));
			Vector3 normalized = portalTravelMatrix.MultiplyVector(rb.velocity).normalized;
			float magnitude = rb.velocity.magnitude;
			Quaternion quaternion2 = portalTravelMatrix.rotation * rotation;
			...
			Matrix4x4 matrix4x = Matrix4x4.TRS(vector, quaternion2, Vector3.one);
			...
			ref Unity.Mathematics.Geometry.Plane plane3 = ref nativePortal2.plane;
			for (int k = 0; k < num5; k++)
			{
				...
				if (reference.distance == 0f)
				{
					...
					plane2 = plane;
					if (Vector3.Dot(velocity2, plane2.Normal) > 0f)
					{
						plane2 = plane;
						if (plane2.SignedDistanceToPoint(vector4) + targetCollider.radius > 0f)
						{
							Rigidbody rigidbody = rb;
							Vector3 velocity3 = rb.velocity;
							plane2 = plane;
							rigidbody.velocity = Vector3.ProjectOnPlane(velocity3, plane2.Normal);
						}
					}
				}
				else
				{
					plane2 = plane3;
					if (!(plane2.SignedDistanceToPoint(reference.point) > 0f) && !(Vector3.Dot(reference.normal, normalized2) >= 0.8f))
					{
			...
			if (!clone2.WasActive)
			{
				clone2.Transform.SetPositionAndRotation(vector, quaternion2);
			}
			else
			{
				Vector3 velocity4 = portalTravelMatrix.MultiplyVector(rb.velocity);
				clone2.Rigidbody.Move(vector, quaternion2);
			...
			ConcurrentDictionary<int, UnityEngine.Plane> concurrentDictionary = colliderPortalPlanes;
			plane2 = plane;
			Vector3 inNormal2 = plane2.Normal;
			plane2 = plane;
			concurrentDictionary[instanceID] = new UnityEngine.Plane(inNormal2, plane2.Distance);
			clone2.ColliderClone.TravelMatrix = travelMatrixManaged;
			PortalAwarePlayerColliderClone colliderClone = clone2.ColliderClone;
			plane2 = plane3;
			Vector3 inNormal3 = plane2.Normal;
			plane2 = plane3;
			colliderClone.PortalPlane = new UnityEngine.Plane(inNormal3, plane2.Distance);
			...
			plane2 = plane3;
			Vector3 inNormal4 = plane2.Normal;
			plane2 = plane3;
			groundCheck.PortalPlane = new UnityEngine.Plane(inNormal4, plane2.Distance);
		}
		portalsToIgnore.Clear();
	}
	...
	private int GetIntersectingPortalHandles(List<PortalHandle> handles)
	{
		...
		nativeScene = scene.nativeScene;
		portalSortCenter = targetCollider.bounds.center;
	...
	private void CreateClone()
	{
		...
		UnityEngine.Object.Destroy(wallCheck.GetComponent<AudioSource>());
		...
		UnityEngine.Object.Destroy(groundCheck.GetComponent<AudioSource>());
		...
		UnityEngine.Object.Destroy(groundCheck2.GetComponent<AudioSource>());
