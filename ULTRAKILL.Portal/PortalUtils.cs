...
public static class PortalUtils
{
	...
	{
		return Vector3.Dot(transform.back, point - transform.centerManaged) > 0f;
	}
	...
	public static bool FindClosestLinkPositions(this PortalIdentifier portalId, Vector3 point, out Vector3 entryPos, out Vector3 exitPos)
	{
		...
		if (portalId.IsValidLinkExist)
		{
			...
			entryPos = portalNavMeshLink.data.startPosition;
			exitPos = portalNavMeshLink.data.endPosition;
			return true;
		}
		...
		{
			return Vector3.Distance(link.data.startPosition, point);
		}
	}
