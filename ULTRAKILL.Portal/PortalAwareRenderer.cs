...
public class PortalAwareRenderer : MonoBehaviour
{
	...
	private int GetIntersectingPortals(List<PortalHandle> portalHandles)
	{
		...
		foreach (NativePortal portal2 in nativeScene.portals)
		{
			...
			Unity.Mathematics.Geometry.Plane plane = portal.plane;
			PortalHandle handle = portal.handle;
			if (plane.SignedDistanceToPoint(portalSortCenter) > 0f && num >= 20f)
			{
				continue;
			}
		...
		if (hasParent && parentPortalHandle.IsValid())
		{
			...
			{
				targetClipPlanes[clipPlaneCount] = nativePortal.plane.NormalAndDistance;
				clipPlaneCount++;
			}
		}
		...
		for (int num5 = 0; num5 < count2; num5++)
		{
			...
			if (nativePortal2.renderData.clippingMethod != PortalClippingMethod.None)
			{
				...
				targetClipPlanes[clipPlaneCount] = nativePortal2.plane.NormalAndDistance;
				clipPlaneCount++;
	...
	private void Think(bool allowChildUpdates = false)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (clone.GameObject.activeSelf)
			{
				...
				Quaternion quaternion2 = Quaternion.LookRotation(forward2, upwards) * Quaternion.Inverse(rotation);
				Vector3 vector = reference.MultiplyPoint3x4(position);
				clone.Transform.SetPositionAndRotation(vector - quaternion2 * position, quaternion2);
	...
	private void DeactivateClones()
	{
		...
		for (int i = 0; i < count2; i++)
		{
			...
			if (!(renderer == null))
			{
				...
				{
					ParticleSystem.MainModule main = particleSystemRenderer.GetComponent<ParticleSystem>().main;
					main.cullingMode = ParticleSystemCullingMode.PauseAndCatchup;
					particleSystemRenderer.forceRenderingOff = false;
				}
	...
	private Clone CreateClone()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				if (!(component3 is ParticleSystemRenderer particleSystemRenderer))
				{
				...
				{
					if (particleSystemRenderer.TryGetComponent<PortalAwareRendererIgnore>(out component2))
					{
						continue;
					}
					if (!particleSystemRenderer.gameObject.TryGetComponent<PortalAwareParticleSystem>(out var _))
					{
						particleSystemRenderer.gameObject.AddComponent<PortalAwareParticleSystem>();
					}
				}
			}
