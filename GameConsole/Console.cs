...
public class Console : MonoSingleton<Console>, plog.Handlers.ILogHandler
{
	...
	{
		Application.logMessageReceived -= HandleUnityLog;
		plog.Logger.Root.RemoveHandler(unityProxyHandler);
		plog.Logger.Root.RemoveHandler(this);
	}
	...
	{
		plog.Logger.Root.AddHandler(unityProxyHandler);
	...
	{
		if (log.StackTrace == null && (ExtractStackTraces || log.Level == Level.Error || log.Level == Level.Exception))
		{
			log = log with
			{
	...
	public void RegisterCommand(ICommand command)
	{
		...
		if (command is IConsoleLogger consoleLogger)
		{
			...
			consoleLogger.Log.AddHandler(unityProxyHandler);
	...
	public void ProcessInput(string text)
	{
		...
		text2 = text2.ToLower();
		Log.CommandLine("> " + text);
		if (text.ToLower() == "sv_cheats 1")
		{
			Log.Warning("To enable cheats, you must enter the Konami code in-game.");
			return;
		}
		if (recognizedCommands.TryGetValue(text2, out var value))
		{
			...
			{
				Log.Error("Command <b>'" + text2 + "'</b> failed.\n" + ex.Message, null, ex.StackTrace);
				return;
			}
		}
		Log.Warning("Unknown command: '" + text2 + "'");
	}
