...
public class SceneHelper : MonoSingleton<SceneHelper>
{
	...
	public void CreateEnviroGibs(Vector3 position, Vector3 direction, float distance, int gibAmount = 3, float sizeMultiplier = 1f)
	{
		...
		asyncOp2.completed += delegate
		{
			...
			if (componentsInChildren != null && componentsInChildren.Length != 0)
			{
				...
				{
					if (componentsInChildren[j] != null && componentsInChildren[j].increaseBurstEmission && componentsInChildren[j].TryGetComponent<ParticleSystem>(out var component) && component.emission.burstCount > 0)
					{
						componentsInChildren[j].transform.localScale /= sizeMultiplier;
						ParticleSystem.MainModule main = component.main;
						main.startSpeed = MultiplyCurve(main.startSpeed, sizeMultiplier);
						ParticleSystem.Burst burst = component.emission.GetBurst(0);
						ParticleSystem.MinMaxCurve count = MultiplyCurve(burst.count, sizeMultiplier);
						burst.count = count;
	...
	public void SetParticlesColors(EnviroGibModifier[] modifiers, Color clr)
	{
		...
		{
			if ((bool)enviroGibModifier.particleSystem)
			{
	...
	private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
	{
		...
		UnityEngine.Object.Instantiate(this.eventSystem);
