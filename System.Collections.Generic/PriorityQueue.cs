...
public class PriorityQueue<TElement, TPriority>
{
	...
	public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, IEnumerable<(TElement Element, TPriority Priority)>, IEnumerable, ICollection
	{
		...
		IEnumerator<(TElement Element, TPriority Priority)> IEnumerable<(TElement, TPriority)>.GetEnumerator()
		{
			...
			return Enumerable.Empty<(TElement, TPriority)>().GetEnumerator();
		}
	...
	public void Enqueue(TElement element, TPriority priority)
	{
		...
		if (_comparer == null)
		{
			MoveUpDefaultComparer((Element: element, Priority: priority), size);
		}
		else
		{
			MoveUpCustomComparer((Element: element, Priority: priority), size);
		}
	}
	...
	public TElement DequeueEnqueue(TElement element, TPriority priority)
	{
		...
		if (_comparer == null)
		{
			...
			{
				_nodes[0] = (Element: element, Priority: priority);
			}
		}
	...
	public void EnqueueRange(IEnumerable<TElement> elements, TPriority priority)
	{
		...
		if (elements is ICollection<TElement> { Count: var count })
		{
			int num = count;
	...
	private int FindIndex(TElement element, IEqualityComparer<TElement>? equalityComparer)
	{
		...
		ReadOnlySpan<(TElement, TPriority)> readOnlySpan = MemoryExtensions.AsSpan(_nodes, 0, _size);
