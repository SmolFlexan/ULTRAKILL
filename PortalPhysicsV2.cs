...
{
	private static readonly plog.Logger Log = new plog.Logger("PortalManagerV2");
			...
			if (scene.FindPortalBetween(vector, end, out var hitPortal, out var intersection, out var _))
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					...
					if (item.distance < num3)
					{
						...
						list2.Add(item);
						Log.Info($"Added physics hit before portal: {item.collider.name} at distance {item.distance}");
					}
				}
				vector = vector3;
				vector2 = vector4;
				...
				list.Add(item2);
				Log.Info($"Traversed portal at {intersection}, new origin {vector}, new direction {vector2}");
				continue;
			}
			for (int j = 0; j < array.Length; j++)
			{
				...
				list2.Add(item3);
				Log.Info($"Added physics hit: {item3.collider.name} at distance {item3.distance}");
			}
			break;
		}
		portalTraversals = list.ToArray();
