...
public class CustomTextures : DirectoryTreeBrowser<FileInfo>
{
	...
	public void SetEditMode(int m)
	{
		...
		if ((bool)gridBtn)
		{
			gridBtn.interactable = editMode != EditMode.Grid;
			gridBtn.GetComponent<ShopButton>().deactivated = editMode == EditMode.Grid;
			skyboxBtn.interactable = editMode != EditMode.Skybox;
			skyboxBtn.GetComponent<ShopButton>().deactivated = editMode == EditMode.Skybox;
			emissionBtn.interactable = editMode != EditMode.Emission;
			emissionBtn.GetComponent<ShopButton>().deactivated = editMode == EditMode.Emission;
			fogBtn.interactable = editMode != EditMode.Fog;
			fogBtn.GetComponent<ShopButton>().deactivated = editMode == EditMode.Fog;
		}
	}
